...
namespace Example
{
	...
	void performActionA( ccMainAppInterface *appInterface )
	{
		...
		appInterface->dispToConsole( "[ExamplePluginDHL] Hello world!", ccMainAppInterface::STD_CONSOLE_MESSAGE );
		
		// Display a warning message in the console
		appInterface->dispToConsole( "[ExamplePluginDHL] Warning: example plugin shouldn't be used as is", ccMainAppInterface::WRN_CONSOLE_MESSAGE );
		
		// Display an error message in the console AND pop-up an error box
		appInterface->dispToConsole( "Example plugin DHL shouldn't be used - it doesn't do anything!", ccMainAppInterface::ERR_CONSOLE_MESSAGE );
