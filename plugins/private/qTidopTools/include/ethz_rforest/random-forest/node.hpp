...
public:
    ...
    {
      IO_Binary::I_Binary_write_bool (os, is_leaf);
      IO_Binary::I_Binary_write_size_t_into_uinteger32 (os, n_samples);
      IO_Binary::I_Binary_write_size_t_into_uinteger32 (os, depth);
      splitter.write(os);

      for (const float& f : node_dist)
        IO_Binary::I_Binary_write_float32 (os, f);
    ...
    {
      IO_Binary::I_Binary_read_bool (is, is_leaf);
      IO_Binary::I_Binary_read_size_t_from_uinteger32 (is, n_samples);
      IO_Binary::I_Binary_read_size_t_from_uinteger32 (is, depth);
      splitter.read(is);

      node_dist.resize(params->n_classes, 0.0f);
      for (std::size_t i = 0; i < node_dist.size(); ++ i)
        IO_Binary::I_Binary_read_float32 (is, node_dist[i]);
