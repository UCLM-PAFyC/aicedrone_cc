...
#include "../include/libParameters/ParametersManager.h"
#include "../include/libParameters/Parameter.h"
...
//    classifiedValues.clear();
         ...
         it != m_ptrPts->end(); ++ it)
    {
        ...
        if(targetOriginalClass==CC_CLASSIFICATION_MODEL_ASPRS_REMOVED_CODE)
        {
            counter++;
            continue;
        }
...
void ccClassificationToolCGAL::setComputeFeaturesParameters(ParametersManager *ptrParametersManager,
                                                            QString &timeInSeconds,
                                                            QString &strError)
{
    m_timeInSeconds=timeInSeconds;
    m_strError=strError;
    m_ptrParametersManager=ptrParametersManager;
}

bool ccClassificationToolCGAL::runComputeFeatures(ParametersManager *ptrParametersManager,
                                                  QString& timeInSeconds,
                                                  QString &strError)
{
    QString functionName="ccClassificationToolCGAL::runComputeFeatures";
    strError.clear();
    if(m_ptrPts==NULL)
    {
        strError=functionName;
    ...
    QString parameterCode=RFC_PROCESS_COMPUTE_FEATURES_NUMBER_OF_SCALES;
    Parameter* ptrParameter=ptrParametersManager->getParameter(parameterCode);
    if(ptrParameter==NULL)
    {
        strError=functionName;
...
void ccClassificationToolCGAL::computeFeatures()
{
    m_success=runComputeFeatures(m_ptrParametersManager,
                               m_timeInSeconds,
                               m_strError);
